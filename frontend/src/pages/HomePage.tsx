    ...
    <>
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="z-10 text-left">
            ...
            <h2 className="font-display-lg text-5xl lg:text-6xl font-bold mb-6 text-on-surface leading-tight tracking-tight">
              Nông sản tươi <br/><span className="text-gradient-green">mỗi ngày</span>
            </h2>
            <p className="font-body-lg text-lg text-on-surface-variant mb-10 max-w-lg leading-relaxed">
              Đặt rau củ, trái cây và đồ thiết yếu. Hệ thống tự chọn cửa hàng phù hợp gần bạn nhất để đảm bảo độ tươi ngon vượt trội.
            </p>
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4 mb-10 max-w-xl">
              <div className="flex-1 flex items-center bg-white shadow-premium px-6 py-4 rounded-full border border-outline-variant/50 focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/10 transition-all duration-300">
                ...
                />
                <button type="submit" className="hidden sm:block px-6 py-2 bg-primary text-white rounded-full font-label-bold hover:bg-primary-container transition-colors shadow-sm">
                  Tìm kiếm
                </button>
              </div>
            </form>
            ...
            <div className="flex flex-wrap gap-4 items-center">
              <Link to="/products" className="px-8 py-3.5 bg-primary text-white rounded-full font-label-bold text-base hover:bg-primary-container shadow-premium hover:shadow-premium-hover hover:-translate-y-0.5 transition-all duration-300 text-center">
                Mua ngay
              </Link>
              <Link to="/orders" className="px-8 py-3.5 bg-white border border-outline-variant text-on-surface rounded-full font-label-bold text-base hover:border-primary hover:text-primary shadow-sm hover:shadow-premium transition-all duration-300 text-center">
                Xem đơn hàng
              </Link>
            </div>
          ...
          <div className="relative lg:ml-auto w-full max-w-lg">
            <div className="rounded-3xl overflow-hidden shadow-soft border border-white/20 transform lg:-rotate-2 hover:rotate-0 transition-transform duration-700 ease-out">
              <img 
                ...
                alt="A lush and vibrant top-down flat-lay photograph of fresh, colorful vegetables and fruits including bright red tomatoes on the vine, deep green bok choy, fresh basil, long carrots with greens, and a bowl of high-quality rice." 
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-10">
          <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
            <div className="text-left">
              <span className="font-label-bold text-primary uppercase tracking-widest text-xs font-semibold">Danh mục</span>
              <h3 className="font-display-lg text-3xl font-bold mt-3 text-on-surface tracking-tight">Mua theo loại nông sản</h3>
              <p className="text-on-surface-variant font-body-md mt-3 text-base max-w-2xl">Chọn danh mục phù hợp, hệ thống giao từ cửa hàng gần bạn.</p>
            </div>
            <Link to="/products" className="group flex items-center gap-2 px-5 py-2.5 bg-surface-container-low border border-transparent rounded-full text-on-surface font-label-bold hover:bg-white hover:border-outline-variant hover:shadow-sm transition-all duration-300 w-fit">
              Xem tất cả
            ...
            {categories ? (
              categories.map((cat) => (
                <Link
                  ...
                  className="group bg-surface-container-low/50 p-6 rounded-2xl border border-outline-variant/50 hover:bg-white hover:shadow-premium hover:border-primary/20 transition-all duration-300 flex justify-between items-center"
                >
              ...
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-surface-container-low p-6 rounded-2xl border border-outline-variant/50 h-[88px]" />
              ))
            )}
          </div>
        </div>
      ...
      <section className="py-24 bg-background">
        <div className="max-w-container-max mx-auto px-6 md:px-10">
          <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 mb-12">
            <div>
              <span className="font-label-bold text-primary uppercase tracking-widest text-xs font-semibold">Nổi bật</span>
              <h3 className="font-display-lg text-3xl font-bold mt-3 text-on-surface tracking-tight">Sản phẩm được yêu thích</h3>
              <p className="text-on-surface-variant font-body-md mt-3 text-base">Tươi ngon, chọn lọc mỗi ngày từ các cửa hàng trong hệ thống.</p>
            </div>
            <Link to="/products" className="bg-primary text-white px-6 py-2.5 rounded-full font-label-bold hover:bg-primary-container shadow-premium hover:shadow-premium-hover transition-all duration-300 w-fit text-center">
              Xem tất cả
            </Link>
          </div>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-white rounded-3xl h-[420px] border border-outline-variant/60" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      ...
      <section className="py-24 bg-white border-t border-outline-variant/30">
        <div className="max-w-container-max mx-auto px-6 md:px-10">
          <div className="text-center max-w-xl mx-auto mb-16">
            <span className="font-label-bold text-primary uppercase tracking-widest text-xs font-semibold">Quy trình</span>
            <h3 className="font-display-lg text-3xl font-bold mt-3 text-on-surface tracking-tight">Mua nông sản chỉ 3 bước</h3>
            <p className="text-on-surface-variant font-body-md mt-3 text-base">Bạn chỉ cần chọn hàng và nhập địa chỉ, phần còn lại để hệ thống lo.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {STEPS.map((step) => (
              <div key={step.num} className="bg-background/40 p-8 rounded-3xl border border-outline-variant/50 relative overflow-hidden group hover:bg-white hover:shadow-premium hover:border-primary/20 transition-all duration-300">
      ...
      <section className="py-20 bg-background border-t border-outline-variant/30">
        <div className="max-w-container-max mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {FEATURES.map((f) => (
              <div key={f.title} className="bg-white p-6 rounded-2xl border border-outline-variant/50 flex gap-4 items-start shadow-sm hover:shadow-premium transition-all duration-300">
        ...
        <div className="max-w-container-max mx-auto px-6 md:px-10">
          <div className="bg-gradient-to-r from-primary to-primary-container rounded-[2.5rem] p-12 md:p-20 text-white relative overflow-hidden shadow-premium">
            ...
            <div className="relative z-10 max-w-2xl">
              <span className="font-label-bold text-xs uppercase tracking-widest bg-white/20 text-white px-4 py-1.5 rounded-full inline-block mb-6">Bắt đầu ngay</span>
              <h3 className="font-display-lg text-4xl md:text-5xl font-bold mb-6 leading-tight">Sẵn sàng ăn sạch, sống khỏe?</h3>
              <p className="text-white/80 font-body-lg text-base md:text-lg mb-10 leading-relaxed">
                Khám phá hàng trăm nông sản tươi ngon, chọn lọc và giao nhanh từ hệ thống cửa hàng nông sản gần bạn nhất.
              </p>
              <div className="flex flex-wrap gap-4">
                <Link to="/products" className="px-8 py-3.5 bg-white text-primary rounded-full font-label-bold text-base hover:bg-secondary-container hover:-translate-y-0.5 transition-all duration-300 text-center shadow-md">
                  Mua sắm ngay
                </Link>
                <Link to="/register" className="px-8 py-3.5 bg-transparent border border-white/40 text-white rounded-full font-label-bold text-base hover:bg-white/10 hover:-translate-y-0.5 transition-all duration-300 text-center">
                  Tạo tài khoản
                </Link>
              </div>
            </div>
